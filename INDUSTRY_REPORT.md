...
| Stage | Hold | Expected return (IRR) | MOIC | Downside | In plain terms |
|---|---|---|---|---|---|
| 1 — Develop & flip (sell ready-to-build) | ~3 yrs | ~0.8% | 1.03× | **can lose capital (−17.6%)** | capital-light, short, but the weakest risk-adjusted entry once all three gates apply |
| 2 — Build & sell | ~1.5 yrs | ~25.9% | 1.41× | **heavy loss (−18.2%)** | highest return, but most fragile |
| 3 — Own & operate (contracted) | ~15 yrs | ~8.1% | 2.41× | **still positive (+4.4%)** | steady, long, capital-preserving |
| 4 — Integrated (develop → build → operate, keep) | ~18 yrs | ~12.2% | 4.41× | **positive (+8.3%) if it reaches operation** | the whole value chain, no exit risk; longest, all risks stacked |
...
- **Stage 1 (develop & flip) is the weakest risk-adjusted entry** — its ~0.8% expected return and −17.6% downside (which impairs capital) make it hard to justify on its own; only on the conditions below.
...
> **Why the gates differ by stage — and why the 65% is only the approval gate.** The manager's headline **40% / 65% / 80%** is the **development-approval (DA) gate alone** — the chance of winning planning approval. A Stage 1 (develop & flip) project then bears two further gates — grid connection (~70%) *and* a sale (~80%) — so its true whole-funnel success is the product: Base case 0.65 × 0.70 × 0.80 = 0.364 (~36%), not 65%. A Stage 2 (build) or Stage 3 (operate) investor **buys a project that has already cleared development**, at its market price, so they bear construction completion (~90%) and merchant price respectively, not the development-success rate. The **integrated path (Stage 4) runs the approval and connection gates plus construction, but without the sale gate** — it keeps the asset rather than selling it, so its development survival is planning × grid connection (≈56%). Each stage is priced at its own entry point, which keeps the returns comparable like-for-like, and the manager's approval-gate figure drives only the Stage 1 result.
...
- its **approval and grid-connection success rates by state**, specifically for sub-5 MW projects, that would substantiate its 65% base-case approval gate — recognising that whole-funnel flip success is the product of approval × connection × sale (Base ≈ 36%, benchmark ≈ 45%), not the approval gate alone;
...
The single biggest risk is the exit: a project with no buyer is stranded capital. Execution and approval risk come next, and the three-gate funnel — where the 65% headline is only the approval gate, leaving true flip success near 36% — compounds both, which is why the develop-and-flip is the weakest risk-adjusted entry on our corrected numbers. The thing that usually sinks battery investments — volatile electricity prices — is, for this fund, the buyer's problem rather than ours.
