...
**This is conditional analysis, not a verdict on the company.** The "−2.6% / 0.88×" headline rests on three `[[TO CONFIRM]]` placeholder inputs — **$8m pre-money, ~25 projects in the pipeline at exit, and pro-rata interim distributions** — and each materially moves the result (see [Appendix C, Exhibit D](#exhibit-d--exit-value-sensitivity-our-expected-return-across-pipeline-depth-at-exit--discount-rate)). It is a finding about the **current terms we would be offered** — chiefly the entry price — not a judgement on the company's viability.
...
**Decision logic — price is the lever.** The verdict is **pass-leaning on the current terms**, driven by **entry price**, not pipeline depth: even a deep pipeline of **40 projects** lifts the expected return only to about **+0.8%** at best ([Exhibit D](#exhibit-d--exit-value-sensitivity-our-expected-return-across-pipeline-depth-at-exit--discount-rate)), so depth alone cannot rescue the deal. Two things would change the verdict: **(a)** a **lower pre-money** (toward ~$5.2m, or milestone-tranched capital) so we stop paying ~$10m for a ~$7.2m company; and **(b)** confirmation of a **deeper forward pipeline at exit and pro-rata interim distributions**. Price is the necessary lever — without (a), neither a deeper pipeline nor distributions makes the central case value-accretive.
...
**Probability-weighted (First-Chicago, 30/50/20 on proceeds, then convert): expected MOIC ≈ 0.88×, IRR ≈ −2.6%** — value-destructive at the central estimate. Scenario IRR range −15% … +4%. For comparison, the founders' own scenarios imply a far higher ~17.7% — the gap is the funnel correction (approval gate alone vs all three gates) compounded by the corrected exit basis (forward pipeline, not a profit multiple). **This headline is conditional**: it rests on the placeholder inputs — $8m pre-money, ~25 projects in the pipeline at exit, and the 0% interim-distribution assumption — each a `[[TO CONFIRM]]` that materially moves the result (see [Exhibit D](#exhibit-d--exit-value-sensitivity-our-expected-return-across-pipeline-depth-at-exit--discount-rate)); it is a finding about the **current terms**, not a fixed verdict on the company.
...
- The **distribution policy** — do we **share pro-rata in any interim distributions**, or is realised programme profit reinvested / skewed to founders and management before exit? (We capture our ~$4.8m share of realised profit only if we do.)
- Given the **entry-price gap** (~$10m post-money vs ~$7.2m base-case value), would the founders **re-price the round (lower pre-money)** or accept **milestone-based capital tranches** as the pipeline converts?
...
- [ ] **Interim distribution fraction / pro-rata sharing** — share of realised profit distributed during the hold (0% assumed, conservative); confirm we share **pro-rata** and that retained profit is not siphoned to founders/management before exit
