# Investment Committee Report — Distribution-Network Battery "Develop-and-Flip" Fund

**Prepared by:** Investment analysis (credit-risk lens) **Date:** June 2026 **Stage:** Final Investment Committee
**Deal type:** Fund commitment (limited partner) — infrastructure development
*Illustrative memo — educational / portfolio use, **not investment advice**. The fund is an unnamed, illustrative develop-and-flip battery energy storage system (BESS) manager; every figure attributed to the manager is a forward-looking claim independently re-derived from free public data and must be verified. Wholesale-investor context; capital is at risk. Acronyms are spelled out on first use.*
...
- **Market size & growth:** storage demand is large and structural. The Australian Energy Market Operator's 2026 Integrated System Plan (Step Change scenario) shows small-scale/distributed storage growing from **5 gigawatts (GW) in 2026 to 12 GW by 2030 and 35 GW by 2050**, with ~45 GW of grid-scale storage already in the connection queue. Coal retirement plus the renewables build-out create a firming gap that batteries fill cheapest and fastest.
- **Key drivers:** net-zero and 82%-renewables-by-2030 targets, state storage schemes, and the sub-5 MW registration exemption that speeds small-project approval.
- **Competitive position / "moat":** the claimed edge is the under-contested ~5 MW distribution niche plus grid and network-operator know-how. *To verify — it is the central diligence question.*
- **Why now — and the catch:** demand is real, **but the capital and deals sit overwhelmingly at the grid-scale (100 MW+) end.** Recent transactions — Summerfield 240 MW, Supernode 260 MW, an Energy Security Corporation platform 200 MW, Ebor 100 MW — are all large; the ~5 MW band is owned mostly by network companies and government programmes. A single 5 MW asset is too small for the infrastructure and superannuation funds, so the fund must **aggregate** projects into a portfolio to reach saleable scale (see §10).

---

## 5. Fund Economics & Pipeline (the funnel)

A development fund has no operating history; its economics are the **funnel** — how many projects must be started to deliver the target, and the margin on each. Base-case figures (independent rebuild):
...
## 8. The Manager's Plan & the Value-Chain Choice
...
*Forward-looking statements are projections, not promises. This is an illustrative, educational document and not investment advice; any real decision rests with the relevant investment committee on independently verified figures.*
